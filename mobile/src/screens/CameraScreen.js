...
export default function CameraScreen() {
  ...
  const takePhoto = async () => {
    ...
    setIsCapturing(true);
    try {
      // Capture photo with base64: false and quality: 1 for real file path
      const photo = await cameraRef.current.takePictureAsync({
        base64: false,
        quality: 1,
      });
      
      // Use the real file path from photo.uri
      const imageUri = photo.uri;
      
      setPreviewUri(imageUri);
      setImageUri(imageUri);
      
      // Automatically process image after capture
  ...
  const handleImagePicker = async () => {
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        allowsEditing: false,
        quality: 1, // Use quality 1 for better OCR results
      });
    ...
    try {
      ...
      if (mode === 'batch') {
        ...
        Alert.alert(
          ...
          'Save this contact to your phone\'s address book?',
          [
            { 
              text: 'Skip', 
              style: 'cancel',
            ...
            },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  if (status === 'granted') {
                    const newContact = {
                      firstName: contactToSave.name.split(' ')[0] || contactToSave.name,
                      lastName: contactToSave.name.split(' ').slice(1).join(' ') || '',
                      company: contactToSave.company || '',
                      emails: contactToSave.email ? [{ label: 'work', email: contactToSave.email }] : [],
                      phoneNumbers: contactToSave.phone ? [{ label: 'mobile', number: contactToSave.phone }] : [],
                    };
  ...
  const handleClearBatch = () => {
    Alert.alert(
      'Clear Batch',
      `Clear all ${batchResults.length} scanned card(s)?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            setBatchResults([]);
            Alert.alert('Success', 'Batch cleared');
          },
        },
      ]
    );
  ...
  return (
    <View style={styles.container}>
      {/* Batch mode indicator and controls */}
      {mode === 'batch' && (
        <View style={styles.batchHeader}>
          <View style={styles.batchInfo}>
            <Text style={styles.batchCounter}>
              {batchResults.length} card{batchResults.length !== 1 ? 's' : ''} scanned
            </Text>
            {batchResults.length > 0 && (
              <Text style={styles.batchReady}>Ready to review</Text>
            )}
          </View>
            ...
            {batchResults.length > 0 && (
              <>
                <TouchableOpacity
                  ...
                  onPress={handleReviewBatch}
                >
                  <Text style={styles.batchButtonText}>📋 Review</Text>
                </TouchableOpacity>
                <TouchableOpacity
